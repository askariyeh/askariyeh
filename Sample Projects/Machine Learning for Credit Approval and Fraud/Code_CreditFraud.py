# -*- coding: utf-8 -*-
"""
Created on Fri Feb 11 13:05:11 2021
...
df = pd.read_csv(f'{in_path}creditcard.csv', header = 0)
...
df_init = df['Class'].value_counts()
df_init = df_init.reset_index()
df_init ['index'] = ['Non-Fraud', 'Fraud']
df_init ['index'] = df_init ['index'].astype(str)
color = df_init['Class'].astype(str)

fig= px.bar(df_init,
            x = 'index',
            y = df_init['Class']/df_init['Class'].sum(),
            color = color,
            title='Credit Card Fraud Rate',
            labels = dict(index = 'Fraud or Non-Fraud',
                          y = 'Percentage',
                          color = 'Fraud or Non-Fraud<br>(Counts)')
            )
fig.update_layout(yaxis = dict(tickformat = ',.0%')) 
fig.write_html(f'{out_path}plots/CreditCardFraud_init.html') 
...
df_non_fraud = df[df['Class'] == 0][:492]
df_normally_dist = pd.concat([df_fraud, df_non_fraud])
df = df_normally_dist.sample(frac=1,
                             random_state=0)
...
# Plot Variables Percentages
df_copy = df.copy()
df_yr = df_copy['Class'].value_counts()
df_yr = df_yr.reset_index()
df_yr ['index'] = ['Non-Fraud', 'Fraud']
df_yr ['index'] = df_yr ['index'].astype(str)
color = df_yr['Class'].astype(str)

fig= px.bar(df_yr,
            x = 'index',
            y = df_yr['Class']/df_yr['Class'].sum(),
            color = color,
            title='Credit Card Fraud Rate',
            labels = dict(index = 'Fraud or Non-Fraud',
                          y = 'Percentage',
...
fig.update_layout(yaxis = dict(tickformat = ',.0%')) 
fig.write_html(f'{out_path}plots/CreditCardFraud_modified.html') 
...
df_pr_rf_ac = df_pr[df_pr['Class'] == 'accuracy']
df_pr_rf_au = df_pr[df_pr['Class'] == 'AUC']

fig = px.line(df_pr_rf_ac, 
              x='N_Estimator', 
              y='f1_score', 
              color='Min Sample Split',
              title ='Accuracy of Random Forest- Credit Card Fraud',
...
fig = px.line(df_pr_rf_au,
              ...
              y='f1_score', 
              color='Min Sample Split',
              title ='AUC for Random Forest- Credit Card Fraud',
...
df_pr_GB_ac = df_pr[df_pr['Class'] == 'accuracy']
df_pr_GB_au = df_pr[df_pr['Class'] == 'AUC']
...
fig = px.scatter_3d(df_pr_GB_ac, 
                    ...
                    title = 'Accuracy of Gradient Boosting Regression- Credit Card Fraud',
                    labels = dict(f1_score = 'Accuracy',
...
fig = px.scatter_3d(df_pr_GB_au, 
                    ...
                    title = 'AUC of Gradient Boosting Regression- Credit Card Fraud',
                    labels = dict(f1_score = 'AUC',
...
n_neighbors =  [2, 3, 5, 10, 15, 20, 25, 30, 35, 40]  # range(5, 30, 1)  5, 10, 15,
...
df_pr_knn_ac = df_pr[df_pr['Class'] == 'accuracy']
df_pr_knn_au = df_pr[df_pr['Class'] == 'AUC']
...
fig = px.line(df_pr_knn_ac, 
              ...
              y='f1_score', 
              color='Weight',
              title ='Accuracy for KNN- Credit Card Fraud',
              labels = dict(f1_score = 'Accuracy',
...
fig = px.line(df_pr_knn_au,
              ...
              y='f1_score', 
              color='Weight',
              title ='AUC for KNN- Credit Card Fraud',
              labels = dict(f1_score = 'AUC',
                            Neighbors = 'Number of neighbors'))
...
df_pr_svm_ac = df_pr[df_pr['Class'] == 'accuracy']
df_pr_svm_au = df_pr[df_pr['Class'] == 'AUC']
...
fig = px.line(df_pr_svm_ac, 
              ...
              y='f1_score', 
              color='C',
              title ='Accuracy of SVM- Credit Card Fraud',
              labels = dict(f1_score = 'Accuracy',
...
fig = px.line(df_pr_svm_au,
              ...
              y='f1_score', 
              color='C',
              title ='AUC for SVM- Credit Card Fraud',
              labels = dict(f1_score = 'AUC',
                            C = 'Regularization<br>Parameter (C)'))
...
df_pr_nn_ac = df_pr[df_pr['Class'] == 'accuracy']
df_pr_nn_au = df_pr[df_pr['Class'] == 'AUC']
...
fig = px.scatter_3d(df_pr_nn_ac, 
                    ...
                    z='f1_score',
                    color='Activation',
                    title ='Accuracy of Neural Network- Credit Card Fraud',
                    labels = dict(f1_score = 'Accuracy',
                                  y = 'Alpha',
